"""Result extraction shared by all Weymouth formulations."""
...
def _pipe_flow_row(pipe_id: str, variables: dict[str, dict[str, Any]]) -> dict[str, Any]:
    row: dict[str, Any] = {"uid": pipe_id, "flow": _value(variables["flow"][pipe_id])}
    if "q_plus" in variables and pipe_id in variables["q_plus"]:
        row["q_plus"] = _value(variables["q_plus"][pipe_id])
    if "q_minus" in variables and pipe_id in variables["q_minus"]:
        row["q_minus"] = _value(variables["q_minus"][pipe_id])
    if "direction" in variables and pipe_id in variables["direction"]:
        row["direction"] = _value(variables["direction"][pipe_id])
    return row
...
def extract_results(data: GasNetworkData, model, variables: dict[str, dict[str, Any]], load_factor: float) -> tuple[dict[str, pd.DataFrame], dict[str, Any], dict[str, Any]]:
    """Read solver values into CSV-ready tables and KPI values."""

    supplies = []
    for row in data.suppliers.itertuples(index=False):
        supplier_id = str(row.uid)
        supplies.append({"uid": supplier_id, "node_id": str(getattr(row, "node_id", "")), "supply": _value(variables["supply"][supplier_id])})
    ...
    for row in data.pipes.itertuples(index=False):
        pipe_id = str(row.uid)
        flows.append(_pipe_flow_row(pipe_id, variables))
