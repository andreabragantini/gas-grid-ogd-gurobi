"""Variables and small helpers for the Weymouth formulations."""
...
def build_variables(model: gp.Model, data: GasNetworkData) -> dict[str, dict[str, gp.Var]]:
    """Create the decision variables used by the formulation."""
    ...
    for pipe_row in data.pipes.itertuples(index=False):
        ...
        variables["flow"][pipe_uid] = model.addVar(lb=-gp.GRB.INFINITY, ub=gp.GRB.INFINITY, name=f"flow[{pipe_uid}]")
        variables["q_plus"][pipe_uid] = model.addVar(lb=0.0, ub=cap, name=f"q_plus[{pipe_uid}]")
        variables["q_minus"][pipe_uid] = model.addVar(lb=0.0, ub=cap, name=f"q_minus[{pipe_uid}]")
        variables["direction"][pipe_uid] = model.addVar(vtype=gp.GRB.BINARY, name=f"y[{pipe_uid}]")
