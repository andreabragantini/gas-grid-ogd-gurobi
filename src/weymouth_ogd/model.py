...
def _build_fallback_solution(data: GasNetworkData, load_factor: float) -> dict[str, Any]:
    ...
    q_plus = [{"uid": str(row.uid), "q_plus": 0.0} for row in data.pipes.itertuples(index=False)]
    q_minus = [{"uid": str(row.uid), "q_minus": 0.0} for row in data.pipes.itertuples(index=False)]
    direction = [{"uid": str(row.uid), "direction": 1.0} for row in data.pipes.itertuples(index=False)]
...
def _fallback_run(data: GasNetworkData, load_factor: float) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any], float, str]:
    ...
    numeric_variables = {
        ...
        "q_plus": {item["uid"]: item["q_plus"] for item in solution["q_plus"]},
        "q_minus": {item["uid"]: item["q_minus"] for item in solution["q_minus"]},
        "direction": {item["uid"]: item["direction"] for item in solution["direction"]},
